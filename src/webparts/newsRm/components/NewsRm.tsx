...
import styles from './NewsRm.module.scss';
import { INewsRmProps } from './INewsRmProps';
...
export default class NewsRm extends React.Component<INewsRmProps,INewsRmState, {}> {
  constructor(props: INewsRmProps, state: INewsRmState) {
    super(props);
    this.state = {
      Items: [],
      TagBasedMoreNews:[],
    ...
    private async GetNews(ItemID) {
      ...
      await NewWeb.lists.getByTitle("News").items.select("ID","Title","Description","Created","Image","Tag","Dept/Title","DetailsPageUrl","SitePageID/Id","TransactionItemID/Id").filter(`ID eq '${ItemID}'`).orderBy("Created",false ).expand("SitePageID","TransactionItemID","Dept")  
    ...
    private async GetTagBasedNews(AppliedTage,Dept,ItemID){
      var reactHandler = this;
  
      await NewWeb.lists.getByTitle("News").items.select("ID","Title","Description","Created","Image","Tag","Dept/Title","DetailsPageUrl","SitePageID/Id").filter(`Tag eq '${AppliedTage}' and IsActive eq '1' and Id ne '${ItemID}'`).orderBy("Created",false ).expand("SitePageID","Dept")  
      .get().then((items) => { // //orderby is false -> decending          
        reactHandler.setState({
  ...
  public render(): React.ReactElement<INewsRmProps> {
    var reactHandler = this;
    var Dt = "";
                ...
                <div className='inner-banner-overlay'></div>
                  <div className='inner-banner-contents'>
                    ...
                    <ul className='breadcums'>
                      ...
                      <li>  <a href="#" style={{pointerEvents:"none"}} data-interception="off">News ReadMore </a> </li>
                    </ul>
                  </div>
                </div>
