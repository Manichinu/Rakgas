...
import styles from './DeptGalleryVm.module.scss';
import { IDeptGalleryVmProps } from './IDeptGalleryVmProps';
...
export default class DeptGalleryVm extends React.Component<IDeptGalleryVmProps,IGalleryVmState, {}> {
  slider2: any; 
  slider1: any;
  public lightGallery: any;
  public constructor(props: IDeptGalleryVmProps, state: IGalleryVmState){    
    super(props);      
    this.state = {               
      Galleryitems: [],
      VideoItemsss:[],
  ...
  private async GetGalleryFilesFolder() {
    ...
    await sp.web.lists.getByTitle("Picture Gallery").items.expand("Folder", "File").top(1000).orderBy("Created", false)
      .select("ID", "Title", "FileRef", "FileSystemObjectType", "FileLeafRef", "Folder/ServerRelativeUrl", "Folder/Name")
      .getAll().then((items) => { // //orderby is false -> decending          
  ...
  public render(): React.ReactElement<IDeptGalleryVmProps> {
    const settings = {
      dots: false,
      arrows: true,
      infinite: false,
    ...
    return (
      <div className={ styles.deptGalleryVm }>
