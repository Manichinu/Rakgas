...
export interface IMyTeamState{
  ...
  var CodeEmp=[];
export default class MyTeam extends React.Component<IMyTeamProps,IMyTeamState, {}> {
  constructor(props: IMyTeamProps, state: IMyTeamState) {
    super(props);
    ...
    private GetCurrentUserName(){  
      ...
      private async GetMyTeamMembersLeaveBalance(emaill) {
        var email = "test@tmax.in";
        try {
          var reactHandler = this;
          let Emplpoyee_code = "";
        ...
        await NewWeb.lists.getByTitle("OrgChart Master").items.select("Title","Name","Email","Designation","Manager_code").filter(`Email eq '${email}' and Employee_status eq 'Active'`)
             .get().then((items) => { // //orderby is false -> decending          
              if (items[0].Title != undefined && items[0].Title != null) {
                Emplpoyee_code = items[0].Title;
                Manager_code = items[0].Manager_code;
                reactHandler.setState({ CurrentEmpCode: Emplpoyee_code });
                reactHandler._getTeamDatas(Emplpoyee_code, Manager_code);  //Emplpoyee_code,Manager_code
                $(".if-no-team").hide();
                $(".if-team-present").show();
    ...
    private async _getTeamDatas(Emplpoyee_code, Manager_code) {
      CodeEmp = [];
      var reactHandler = this;
  
      await NewWeb.lists.getByTitle("OrgChart Master").items.select("Title", "Name", "Email", "Designation", "Manager_code", "UserProfileUrl", "Annual_Leave_Balance").top(5000).filter(`Manager_code eq '${Emplpoyee_code}' and Employee_status eq 'Active'`).orderBy("Title", true)
        .get().then((items) => { // //orderby is false -> decending          
          reactHandler.setState({
            Items: items
          });
          ...
          for (var i = 0; i < items.length; i++) {
            CodeEmp.push({ "EmpCode": "" + items[i].Title + "" });
            reactHandler.CheckTeamAvailability(items[i].Title);
          }
    ...
    const MyTeams: JSX.Element[] = reactHandler.state.Items.map(function(item,key) {   
        ...
        return (
            <li className="clearfix relative pareb">
                ...
                <a href="#" onClick={()=> reactHandler.ExpandTeamMember(item.Title,Name)} className="expand-myteam-leave" data-interception="off">
                    <span className="services-right-arrow" id={item.Title+"Item"}>                        
                        <img src={`${reactHandler.props.siteurl}/SiteAssets/Remo%20Portal%20Assets/img/arrow_down.svg`} className="transition" alt="image"></img>                        
                    </span>
                </a>
                    ...
                    <div className="sec if-team-present" style={{display:"none"}}>
                      <div className = "refresh-wrap clearfix">
                        ...
                        <div className="refresh-people" style={{float:"right"}}>
                          <a href="#" title="Refresh"><i className="fa fa-refresh" aria-hidden="true" onClick={()=>reactHandler.GetCurrentUserName()} data-interception="off"></i></a>
                        </div>
                      </div>                            
