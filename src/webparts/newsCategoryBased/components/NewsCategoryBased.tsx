...
import styles from './NewsCategoryBased.module.scss';
import { INewsCategoryBasedProps } from './INewsCategoryBasedProps';
...
export default class NewsCategoryBased extends React.Component<INewsCategoryBasedProps,INewsCategoryBasedState, {}> {
  constructor(props: INewsCategoryBasedProps, state: INewsCategoryBasedState) {
    super(props);
    ...
    public async GetAvailableDepts() {
      ...
      await NewWeb.lists.getByTitle("News").items.select("ID", "Dept/Id", "Dept/Title", "Image").filter(`IsActive eq '1'`).expand("Dept").get()
        .then((items) => {
    ...
    public async GetCategoryBasedNews(Mode, AppliedTage, Dept) {
      ...
      if (Mode == "TagBased") {
        reactHandler.setState({ CurrentPage: AppliedTage });
        var APIUrl = await NewWeb.lists.getByTitle("News").items.select("ID", "Title", "Description", "Created", "Dept/Title", "Image", "Tag",
          "DetailsPageUrl", "SitePageID/Id", "TransactionItemID/Id").filter(`IsActive eq '1' and Tag eq '${AppliedTage}'`).orderBy("Created",
            false).expand("Dept", "SitePageID", "TransactionItemID").get()
        reactHandler.GetAllOtherRelatedNews(AppliedTage, 'TagBased');
      } else {
        reactHandler.setState({ CurrentPage: Dept });
        var APIUrl = await NewWeb.lists.getByTitle("News").items.select("ID", "Title", "Description", "Created", "Dept/Title", "Image",
          "Tag", "DetailsPageUrl", "SitePageID/Id", "TransactionItemID/Id").filter(`IsActive eq '1' and Dept/Title eq '${Dept}'`).orderBy("Created",
            false).expand("Dept", "SitePageID", "TransactionItemID").get()
        reactHandler.GetAllOtherRelatedNews(Dept, 'DeptBased');
      }
      reactHandler.setState({
      ...
      const TotalNews: number = APIUrl.length;
      const Count: number = TotalNews / 2;
      const PageCount: number = parseInt(Count.toFixed());
      reactHandler.setState({ TotalPageCount: PageCount });
    }
    ...
    // public async GetAllOtherRelatedNews(ReleventCategory, Mode) {
    //   var reactHandler = this;
    //   if (Mode == 'TagBased') {
    //     for (var i = 0; i < reactHandler.state.AvailableTags.length; i++) {
    ...
    //           $('.available-depts-or-tags').append(`<li>
    //                     <a href="${reactHandler.props.siteurl}/SitePages/News-CategoryBased.aspx?env=WebViewList&Mode=TagBased&Tag=${items[0].Tag}" data-interception='off' className="clearfix"> 
    //                     <div class="vategory-news-left pull-left">
    //                         ${items[0].Tag}
    //                     </div>     
    //                     <div class="vategory-news-right pull-right">
    ...
    //           $('.available-depts-or-tags').append(`<li>
    //                     <a href="${reactHandler.props.siteurl}/SitePages/News-CategoryBased.aspx?env=WebViewList&Mode=DeptBased&Dept=${items[0].Dept.Title}" data-interception='off' class="clearfix">  
    //                       <div class="vategory-news-left pull-left">
    //                           ${items[0].Dept.Title}
    //                       </div>     
    //                       <div class="vategory-news-right pull-right">
  ...
  public render(): React.ReactElement<INewsCategoryBasedProps> {
    ...
    return (
      <div className={ styles.newsCategoryBased }>
       <section>
