...
import styles from './Hseeventrm.module.scss';
import { IHseeventrmProps } from './IHseeventrmProps';
...
export default class NewsRm extends React.Component<IHseeventrmProps,INewsRmState, {}> {
  constructor(props: IHseeventrmProps, state: INewsRmState) {
    super(props);
    this.state = {
      Items: [],
      TagBasedMoreNews:[],
    ...
    var reactHandler = this;
      $.ajax({
        url: `${this.props.siteurl}/_api/web/lists/getbytitle('HSE Event')/items?$select=ID,Title,Description,Created,Image,Tag,Dept/Title,DetailsPageUrl,SitePageID/Id,TransactionItemID/Id&$filter=ID eq ${ItemID}&$orderby=Created desc&$expand=SitePageID,TransactionItemID,Dept`,  
        type: "GET",
    ...
    public GetTagBasedNews(AppliedTage,Dept,ItemID){
      ...
      $.ajax({
        ...
        url: `${this.props.siteurl}/_api/web/lists/getbytitle('HSE Event')/items?$select=ID,Title,Description,Created,Image,Tag,Dept/Title,DetailsPageUrl,SitePageID/Id&$filter=Tag eq '${AppliedTage}' and IsActive eq 1 and Id ne ${ItemID}&$orderby=Created desc&$expand=SitePageID,Dept`,  
        type: "GET",
        headers:{'Accept': 'application/json; odata=verbose;'},
        success: function(resultData) {          
          reactHandler.setState({
  ...
  public render(): React.ReactElement<IHseeventrmProps> {
    var reactHandler = this;
    var Dt = "";
    ...
    return (
      <div className={ styles.hseeventrm }>
         <section>
                ...
                <div className='inner-banner-overlay'></div>
                  <div className='inner-banner-contents'>
                    ...
                    <ul className='breadcums'>
                      <li>  <a href={`${reactHandler.props.siteurl}/SitePages/Homepage.aspx?env=WebView`} data-interception="off"> Home </a> </li>
                      <li>  <a href={`${reactHandler.props.siteurl}/SitePages/HSEVM.aspx?env=WebView`} data-interception="off"> All HSE Updates </a> </li>
                      <li>  <a href="#" style={{pointerEvents:"none"}} data-interception="off">HSE Updates ReadMore </a> </li>
                    </ul>
                  </div>
                </div>
                    ...
                    <div className='col-md-4 sub-news-section'>
                      <div className='heading clearfix'>
                        <a href={`${reactHandler.props.siteurl}/SitePages/News-CategoryBased.aspx?env=WebView&Mode=TagBased&Tag=${this.state.Tag}`} data-interception='off' onClick={()=>reactHandler.GetMoreNewsSpecifictoTag(this.state.Tag)}>
                          More HSE Updates on {this.state.Tag}
                        </a>
                      </div> 
