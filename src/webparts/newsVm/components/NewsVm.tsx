...
import styles from './NewsVm.module.scss';
import { INewsVmProps } from './INewsVmProps';
...
let DeptNews = [];
let rawarr = [];
export default class NewsVm extends React.Component<INewsVmProps,INewsVmState, {}> {
  constructor(props: INewsVmProps, state: INewsVmState) {
    super(props);
    this.state = {
      Items: [],
      RecentNewsItems: [],
   ...
   //   $('#RecommendedItems').attr('style', 'display: none !important');
    ...
    private async GetAllRecentNews(ID) {
      ...
      var reactHandler = this;
      await NewWeb.lists.getByTitle("News").items.select("ID","Title","Description","Created","Image","Tag","Dept/Title","DetailsPageUrl","SitePageID/Id","TransactionItemID/Id")
        .filter(`IsActive eq '1' and and ID ne '${ID}'`).orderBy("Created", false).expand("SitePageID","TransactionItemID","Dept").top(4).get().then((items) => { // //orderby is false -> decending          
          reactHandler.setState({
    ...
    private async GetAllTopNews() {
      ...
      await NewWeb.lists.getByTitle("News").items.select("ID","Title","Description","Created","Image","Tag","Dept/Title","DetailsPageUrl","SitePageID/Id","TransactionItemID/Id","PageViewCount")
        .filter(`IsActive eq '1' and and Created gt '${dateFrom}'`).orderBy("PageViewCount", false).expand("SitePageID","TransactionItemID","Dept").get().then((items) => { // //orderby is false -> decending          
          if(items.length != 0){
            $(".top-news-block-current-month").show();
            reactHandler.setState({
    ...
    private async GetWeekOldNews() {
      ...
      await NewWeb.lists.getByTitle("News").items.select("ID","Title","Description","Created","Image","Tag","Dept/Title","DetailsPageUrl","SitePageID/Id","TransactionItemID/Id")
        .filter(`IsActive eq '1' and and Created lt '${WkDate}'`).orderBy("Created", false).expand("SitePageID","TransactionItemID","Dept").top(20).get().then((items) => { // //orderby is false -> decending          
          if(items.length != 0){
            $(".PastNewsData").show();
            reactHandler.setState({
              OneWkOldNews:items
    ...
    private async GetAllNewsAvailableDepartments() {
      ...
      await NewWeb.lists.getByTitle("News").items.select("ID","Dept/Id","Dept/Title","Image")
        .filter(`IsActive eq '1'`).orderBy("Created", false).expand("Dept").get().then((items) => { // //orderby is false -> decending          
          for(var i = 0; i < items.length; i++){
            ...
            DeptNames.push(DeptName);
                ...
                if(RawImageTxt != "" && RawImageTxt != null){              
                  var ImgObj = JSON.parse(RawImageTxt);       
                  var PicUrl = ImgObj.serverRelativeUrl;
                  NewsAvailableDepts.push({"ID" :items[i].Dept.Id , "Title" :items[i].Dept.Title , "URL" : PicUrl});                     
                }			          
              }
            }           
          }       
    ...
    for(var j = 0; j < this.state.AvailableDepts.length;){
      ...
      await NewWeb.lists.getByTitle("News").items.select("ID","Title","Description","Created","Image","Tag","Dept/Title","DetailsPageUrl","SitePageID/Id","TransactionItemID/Id")
        .filter(`IsActive eq '1' and Dept/Id eq '${DeptID}'`).orderBy("Created", false).expand("SitePageID","TransactionItemID","Dept").top(4).get().then((items) => { // //orderby is false -> decending          
          for(var i = 0; i < items.length;){
            $("#"+CustomID+"").append(`<li><a href="${items[i].DetailsPageUrl}?env=WebViewList&ItemID=${items[i].ID}&AppliedTag=${items[i].Tag}&Dept=${items[i].Dept.Title}&SitePageID=${items[i].SitePageID.Id}" data-interception="off"><p>${items[i].Title}</p></a></li>`);
            i++;
          } 
          j++;
  ...
  public render(): React.ReactElement<INewsVmProps> {
    const settings = {
      dots: false,
      arrows: true,
      infinite: false,
