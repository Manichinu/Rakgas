...
import styles from './Hseeventvm.module.scss';
import { IHseeventvmProps } from './IHseeventvmProps';
...
export interface IHseeventState{
  Items:any[];
  RecentNewsItems:any[];
...
let DeptNews = [];
let rawarr = [];
export default class Hseevent extends React.Component<IHseeventvmProps,IHseeventState, {}> {
  constructor(props: IHseeventvmProps, state: IHseeventState) {
    super(props);
    this.state = {
      Items: [],
      RecentNewsItems: [],
    ...
    private GetAllNews() {
      var reactHandler = this;
      $.ajax({
        url: `${this.props.siteurl}/_api/web/lists/getbytitle('HSE Event')/items?$select=ID,Title,Description,Created,Image,Tag,Dept/Title,DetailsPageUrl,SitePageID/Id,TransactionItemID/Id&$filter=IsActive eq 1&$orderby=Created desc&$expand=SitePageID,TransactionItemID,Dept&$top=1`,  
        type: "GET",
    ...
    private GetAllRecentNews(ID) {
      var reactHandler = this;
      $.ajax({
        url: `${this.props.siteurl}/_api/web/lists/getbytitle('HSE Event')/items?$select=ID,Title,Description,Created,Image,Tag,Dept/Title,DetailsPageUrl,SitePageID/Id,TransactionItemID/Id&$filter=IsActive eq 1 and ID ne ${ID}&$orderby=Created desc&$expand=SitePageID,TransactionItemID,Dept&$top=4`,  
        type: "GET",
        headers:{'Accept': 'application/json; odata=verbose;'},
        success: function(resultData) {              
          reactHandler.setState({
    ...
    private GetAllTopNews() {
      ...
      $.ajax({
        url: `${this.props.siteurl}/_api/web/lists/getbytitle('HSE Event')/items?$select=ID,Title,Description,Created,Image,Tag,Dept/Title,DetailsPageUrl,SitePageID/Id,TransactionItemID/Id,PageViewCount&$filter=IsActive eq 1 and Created gt '${dateFrom}' &$orderby=PageViewCount desc&$expand=SitePageID,TransactionItemID,Dept`,  
        type: "GET",
        headers:{'Accept': 'application/json; odata=verbose;'},
        success: function(resultData) {  
    ...
    public GetWeekOldNews(){
      ...
      $.ajax({
        url: `${this.props.siteurl}/_api/web/lists/getbytitle('HSE Event')/items?$select=ID,Title,Description,Created,Image,Tag,Dept/Title,DetailsPageUrl,SitePageID/Id,TransactionItemID/Id&$filter=IsActive eq 1 and Created lt '${WkDate}'&$orderby=Created desc&$expand=SitePageID,TransactionItemID,Dept&$top=20`,  
        type: "GET",
        headers:{'Accept': 'application/json; odata=verbose;'},
        success: function(resultData) {    
          if(resultData.d.results.length != 0){
    ...
    private GetAllNewsAvailableDepartments() {    
      ...
      var reactHandler = this;
      $.ajax({
        url: `${this.props.siteurl}/_api/web/lists/getbytitle('HSE Event')/items?$select=ID,Dept/Id,Dept/Title,Image&$filter=IsActive eq 1&$orderby=Created desc&$expand=Dept`,  
        type: "GET",
        headers:{'Accept': 'application/json; odata=verbose;'},
        success: function(resultData) {              
    ...
    public GetDeptNews(){
     ...
     for(var j = 0; j < this.state.AvailableDepts.length;){
      ...
      if(DeptID != "" || DeptID != undefined || DeptID != null){
        $.ajax({
          url: `${this.props.siteurl}/_api/web/lists/getbytitle('HSE Event')/items?$select=ID,Title,Description,Created,Image,Tag,Dept/Title,DetailsPageUrl,SitePageID/Id,TransactionItemID/Id&$filter=IsActive eq 1 and Dept/Id eq '${DeptID}'&$orderby=Created desc&$expand=SitePageID,TransactionItemID,Dept&$top=4`,  
          type: "GET",
          async:false,
          headers:{'Accept': 'application/json; odata=verbose;'},
          success: function(resultData) {                              
            for(var i = 0; i < resultData.d.results.length;){
              $("#"+CustomID+"").append(`<li><a href="${resultData.d.results[i].DetailsPageUrl}?ItemID=${resultData.d.results[i].ID}&AppliedTag=${resultData.d.results[i].Tag}&Dept=${resultData.d.results[i].Dept.Title}&SitePageID=${resultData.d.results[i].SitePageID.Id}&env=WebView" data-interception="off"><p>${resultData.d.results[i].Title}</p></a></li>`);
              i++;
            } 
            j++;
          },
    ...
    public SampleNextArrow(props) {
      ...
      return (        
        <a href="#" className={className} onClick={onClick}> <img src={`${this.props.siteurl}/SiteAssets/Remo%20Portal%20Assets/img/right.svg`} alt="image" data-interception="off" /> </a>
      );
    }
    ...
    public SamplePrevArrow(props) {
      const { className, style, onClick } = props;
      return (
        <a href="#" className={className} onClick={onClick}> <img src={`${this.props.siteurl}/SiteAssets/Remo%20Portal%20Assets/img/left.svg`} alt="image" data-interception="off"/> </a>
      );
    }
  ...
  public render(): React.ReactElement<IHseeventvmProps> {
    const settings = {
      dots: false,
      arrows: true,
      infinite: false,
    ...
    const TopNewsBasedonViews: JSX.Element[] = this.state.ViewBasedTopNews.map(function(item,key) {
      ...
      return (
          <li> 
            ...
            <a href={`${item.DetailsPageUrl}?ItemID=${item.ID}&AppliedTag=${item.Tag}&Dept=${item.Dept.Title}&SitePageID=${item.SitePageID.Id}&env=WebView`} data-interception="off" className="nw-list-main top-news-a"> {item.Title} </a>
            <div className="ns-tag-duration ">
                <a href={`${reactHandler.props.siteurl}/SitePages/News-CategoryBased.aspx?env=WebView&Mode=TagBased&Tag=${item.Tag}`} data-interception='off' className="tags"> {item.Tag} </a> 
            </div>
          </li>
    ...
    return (
      <div className={ styles.hseeventvm }>
        <section>
              ...
              <div className="inner-banner-header relative m-b-20">
                ...
                <div className="inner-banner-contents">
                  ...
                  <ul className="breadcums">
                    ...
                    <li>  <a href="#" style={{pointerEvents:"none"}} data-interception="off"> All HSE Updates </a> </li>
                  </ul>
                </div>
              </div>
